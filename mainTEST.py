...
# 2009 and split up the data into an object so that it can be used more easily later. Specifically, each day of the
...
def dataFrom2007to2009():
    ...
    for line in fileIn:
        ...
        if i == time(hour= 9, minute= 30, second= 00):
            stock = Stock(dateParam=info[0][:10])
            stock.addTimeInterval0Price(timeInterval0Price=float(info[4]))
        ...
        if time(hour= 10, minute= 48, second= 00) < i <= time(hour= 12, minute= 6, second= 00):
            stock.addTimeInterval1Price(timeInterval1Price=float(info[4]))
        if time(hour= 12, minute= 6, second= 00) < i <= time(hour= 13, minute= 24, second= 00):
            stock.addTimeInterval2Price(timeInterval2Price=float(info[4]))
        if time(hour= 13, minute= 24, second= 00) < i <= time(hour= 14, minute= 42, second= 00):
            stock.addTimeInterval3Price(timeInterval3Price=float(info[4]))
        if time(hour= 14, minute= 42, second= 00) < i < time(hour= 16, minute= 00, second= 00):
            stock.addTimeInterval4Price(timeInterval4Price=float(info[4]))
...
# input: none
# output: a list of objects, with each object in the list representing data from a single day in the stock market
# description: The purpose of this function is to parse through the data containing data from the S&P 500 from 2010 to
# 2019 and split up the data into an object so that it can be used more easily later. Specifically, each day of the
# stock market is being split up into 5, 77-78 minute time intervals, those being from (in EST) 9:30 AM - 10:48 AM,
# 10:49 AM - 12:06 PM, 12:07 PM - 1:24 PM, 1:25 PM - 2:42 PM, and 2:42 PM - 4:00 PM. The reason the data is being
# split up is because later on, I associate a piece-wise function with each day of the stock market, with a function
# being prescribed to each interval of the day. The data is in a CSV file, that contains data for each minute of
...
def dataFrom2010to2019():
    ...
    for line in fileIn:
        ...
        if i == time(hour=16, minute=00, second=00):
            stock.updateClosingPrice(closingPrice=float(info[4]))
            stock.addTimeInterval4Price(timeInterval4Price=float(info[4]))
            listOfStocks2.append(stock)
        if time(hour=16, minute=00, second=00) < i < time(hour=17, minute=00, second=00):
            stock.updateClosingPrice(closingPrice=float(info[4]))
    fileIn.close()

    return listOfStocks2

# input: none
# output: a list of objects, with each object in the list representing data from a single day in the stock market
# description: The purpose of this function is to parse through the data containing data from the S&P 500 from 2020
# and split up the data into an object so that it can be used more easily later. Specifically, each day of the
# stock market is being split up into 5, 77-78 minute time intervals, those being from (in EST) 9:30 AM - 10:48 AM,
# 10:49 AM - 12:06 PM, 12:07 PM - 1:24 PM, 1:25 PM - 2:42 PM, and 2:42 PM - 4:00 PM. The reason the data is being
# split up is because later on, I associate a piece-wise function with each day of the stock market, with a function
# being prescribed to each interval of the day. The data is in a CSV file, that contains data for each minute of
...
# 5th cplumn of the file.
def dataFrom2020():
    fileIn = open("SPX_2020_2020.csv", "r")
    listOfStocks3 = []
    print("Parsing through data from 2020...")
    ...
    for line in fileIn:
        ...
        if i == time(hour=16, minute=00, second=00):
            stock.updateClosingPrice(closingPrice=float(info[4]))
            stock.addTimeInterval4Price(timeInterval4Price=float(info[4]))
            listOfStocks3.append(stock)
        if time(hour=16, minute=00, second=00) < i < time(hour=17, minute=00, second=00):
            stock.updateClosingPrice(closingPrice=float(info[4]))
    fileIn.close()

    return listOfStocks3
...
def main():
    ...
    bigListofStocks = dataFrom2007to2009() + dataFrom2010to2019() + dataFrom2020()

    differenceFromPreviousCloseList = []
    i = 1
    print("Converting stock prices to difference from previous close...")
    # this loop converts the historical data from being based on the value of the S&P 500 to it being based on the
    # difference from the previous close (in points) to make it easier to compare historical data to current data,
    # and creates a new list of objects that is based on data of the difference from the previous close
    while i < len(bigListofStocks):
        stockDifference = StockDifference(dateParam= bigListofStocks[i].getDate())
        price = bigListofStocks[i-1].getClosingPrice()
        c = 0
        while c < len(bigListofStocks[i].getTimeInterval0Prices()):
            stockDifference.addTimeInterval0Price(float("{:.2f}".format(((((bigListofStocks[i].getTimeInterval0Prices()[c]) - (price))/(price))*100))))
            c += 1
        h = 0
        while h < len(bigListofStocks[i].getTimeInterval1Prices()):
            stockDifference.addTimeInterval1Price(
                float("{:.2f}".format(((((bigListofStocks[i].getTimeInterval1Prices()[h]) - (price))/(price))*100))))
            h += 1
        g = 0
        while g < len(bigListofStocks[i].getTimeInterval2Prices()):
            stockDifference.addTimeInterval2Price(
                float("{:.2f}".format(((((bigListofStocks[i].getTimeInterval2Prices()[g]) - (price))/(price))*100))))
            g += 1
        j = 0
        while j < len(bigListofStocks[i].getTimeInterval3Prices()):
            stockDifference.addTimeInterval3Price(
                float("{:.2f}".format(((((bigListofStocks[i].getTimeInterval3Prices()[j]) - (price))/(price))*100))))
            j += 1
        l = 0
        while l < len(bigListofStocks[i].getTimeInterval4Prices()):
            stockDifference.addTimeInterval4Price(
                float("{:.2f}".format(((((bigListofStocks[i].getTimeInterval4Prices()[l]) - (price))/(price))*100))))
            l += 1
        differenceFromPreviousCloseList.append(stockDifference)
        i += 1
    ...
    differenceToFunction(differenceFromPreviousCloseList)
    areaBetweenRecentCurveAndEveryOtherCurve(differenceFromPreviousCloseList)
    threeDayIntervalsList = ThreeDayIntervalAreas(differenceFromPreviousCloseList)

    areaList = listFromLeastToGreatestAreas(differenceFromPreviousCloseList)

    threeDayAreaList = ThreeDayIntervalListSort(threeDayIntervalsList)

    Graph(differenceFromPreviousCloseList, areaList, threeDayAreaList, threeDayIntervalsList)
